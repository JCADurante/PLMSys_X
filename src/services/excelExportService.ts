...
export async function exportAllDataToExcel(): Promise<void> {
  ...
  const setsData = sets
    ...
    .map(s => ({
      ...
      'Finish': s.finish || 'Glossy',
      'Number of Outs': s.numberOfOuts || 32,
      'Construction': s.construction || s.displayName,
      'Status': s.status,
  ...
  const positionsData = positions
    ...
    .map(pos => {
      ...
      return {
        ...
        'Finish': plate?.finish || set?.finish || 'Glossy',
        'Number of Outs': plate?.numberOfOuts || set?.numberOfOuts || 32,
        'Construction': plate?.construction || set?.construction || set?.displayName || '—',
  ...
  const instData = installations
    ...
    .map(inst => {
      ...
      return {
        ...
        'Finish': inst.finish || plate?.finish || set?.finish || 'Glossy',
        'Number of Outs': inst.numberOfOuts || plate?.numberOfOuts || set?.numberOfOuts || 32,
        'Construction': inst.construction || plate?.construction || set?.construction || set?.displayName || '—',
  ...
  const removalData = removals
    ...
    .map(rem => {
      ...
      return {
        ...
        'Finish': rem.finish || plate?.finish || set?.finish || 'Glossy',
        'Number of Outs': rem.numberOfOuts || plate?.numberOfOuts || set?.numberOfOuts || 32,
        'Construction': rem.construction || plate?.construction || set?.construction || set?.displayName || '—',
        'Set': set ? set.displayName : '—',
        'Position': pos ? pos.fullCode : '—',
