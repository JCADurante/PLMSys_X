...
import { Download, Upload, Shield, Database, Clock, Activity, AlertCircle, RefreshCw, CheckCircle2, X, Wifi, Server, HardDrive, Folder, Check, Terminal } from 'lucide-react';
...
interface AdminDashboardProps {
  ...
  onDataChanged?: () => void;
  initialTab?: 'maintenance' | 'network' | 'database';
}
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  const [adminSubTab, setAdminSubTab] = useState<'maintenance' | 'network' | 'database'>(initialTab);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[#8E9299]">System maintenance, network collaboration, software updates, and database studio</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 bg-[#0F1117] p-1 rounded-xl border border-[#1E222A] overflow-x-auto">
          ...
          <button
            type="button"
            onClick={() => setAdminSubTab('network')}
            className={`flex items-center gap-2 px-3.5 py-2 rounded-lg text-xs font-bold uppercase tracking-wider transition-all cursor-pointer whitespace-nowrap ${
              adminSubTab === 'network'
                ? 'bg-[#F27D26] text-white shadow-md shadow-[#F27D26]/20'
                : 'text-[#8E9299] hover:text-white hover:bg-[#191D28]'
          ...
          >
            <div className="p-6 border-b border-[#1E222A] bg-[#191D28]/50">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-white uppercase text-sm tracking-wider">System Auto-Monitoring & Locations</h3>
              </div>
            </div>
            <div className="p-6 space-y-6">
      ...
      {/* SUB-TAB 2: MULTI-PC NETWORK & SOFTWARE UPDATES */}
      {adminSubTab === 'network' && (
